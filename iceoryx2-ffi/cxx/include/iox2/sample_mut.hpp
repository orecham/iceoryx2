...
class SampleMut {
  public:
    ...
    /// Returns loaned memory accessible in a const Slice instance. Only enabled for types Slice<T>.
    template <typename T = Payload, typename = std::enable_if_t<iox::IsSlice<T>::VALUE, void>>
    auto payload_slice() const -> const Payload;

    /// Returns loaned memory accessible in a Slice instance. Only enabled for types Slice<T>.
    template <typename T = Payload, typename = std::enable_if_t<iox::IsSlice<T>::VALUE, void>>
