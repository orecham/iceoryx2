...
cmake -S . -B target/ffi/cxx/build $CMAKE_BUILD_TYPE_FLAG -DBUILD_EXAMPLES=OFF -DBUILD_TESTING=ON -DCMAKE_INSTALL_PREFIX=target/ffi/cxx/install -DCMAKE_PREFIX_PATH="$( pwd )/target/iceoryx/install"
...
target/ffi/cxx/build/tests/iceoryx2-c-tests
...
rm -rf target/ffi/cxx/build
cmake -S examples/c -B target/ffi/out-of-tree-c $CMAKE_BUILD_TYPE_FLAG -DCMAKE_PREFIX_PATH="$( pwd )/target/ffi/cxx/install"
